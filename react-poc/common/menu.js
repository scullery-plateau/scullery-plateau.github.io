namespace('sp.common.Menu', () => {
  ...
  const SubMenu = function (props) {
    ...
    return (
      <li key={`${id}-li`} className="dropdown-submenu">
        <a
          key={`${id}-a`}
          href="#"
  ...
  const GroupMenu = function (props) {
    ...
    return (
      <li key={`${id}-li`} className="dropdown-submenu">
        ...
        <ul
          key={`${id}-ul`}
          className={`dropdown-menu rpg-box ${
  ...
  const DropDownMenu = function (props) {
    return (
      <ul key={props.id} className="dropdown-menu rpg-box">
        {' '}
        {props.items.map((item) => DropDownItem(item))}
      </ul>
    );
  };
