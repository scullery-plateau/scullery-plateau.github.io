namespace('Minifier',
    ...
    return function() {
        ...
        const loadImage = function() {
            const { size } = state;
            LoadFile(true,"dataURL",(dataURL,filename) => {
                setState({ size, minis: [].concat(state.minis,[{filename,dataURL,count:1}]) });
            },
            (filename,error) => {
                console.log({filename,error});
                alert(filename + " failed to load. See console for error.")
        ...
        const menuItems = [{
            ...
            items:[{
                ...
                callback: () => { PrintMinis.printMiniPages("Print Minifier",state.size,state.minis) }
            }]
        }, {
            id: "removeZeroCount",
            label: "Remove Zero Count",
            callback: () => { removeZeroCount() }
        }, {
            id:"sizePicker",
            label:"Size",
            getter:() => state.size,
                    ...
                    { state.minis.map((thumb,index) => {
                        return <div className="thumbnail rpg-box d-flex flex-column">
                            ...
                            <div className="frame align-self-center"
                                 style={{ backgroundImage: `url(${thumb.dataURL})` }}></div>
                            <input
