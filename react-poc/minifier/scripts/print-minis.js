namespace("PrintMinis",["PrintJS"],({PrintJS}) => {
    ...
    const shaper = {
        ...
        path:function() {
            const points = Array.from(arguments).map(a => a.join(',')).join(" ");
    ...
    const buildDefs = function(size,files) {
        ...
        const frame = `<g id="frame${size}">` + svg.filter((shape) => (shape[0] !== 'image')).map((shape) => {
            return shaper[shape[0]].apply(null,shape.slice(1));
        }).join('\n') + '</g>';
        const images = svg.filter((shape) => (shape[0] === 'image')).map(i => i.slice(1));
        const content = Object.entries(files).map(([filename,url]) => {
            const imageGroup = images.map(([x,y,width,height,transform]) => {
