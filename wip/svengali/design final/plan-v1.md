# Svengali Implementation Plan v1
...
**Selected layer outline**: The actively-selected layer in the card display shows an outline using the same rainbow diagonal `linearGradient` used in Spritely (`clearedGradient`: red → yellow → green → cyan → blue → magenta, stop-opacity 0.2 at 20% offsets, drawn as a `<rect>` with `fill="url(#clearedGradient)"` and no stroke at the frame's position/size). Define the gradient `<defs>` in `index.html`.
...
`constants.js` (`sp.svengali.Constants`) is **only** created for constants used across **multiple** namespaces (e.g. card pixel dimensions used by both `cardRenderer.js` and `printCards.js`). Constants private to a single namespace are declared locally at the top of that namespace's factory function.
...
- **Value mode**: shows the appropriate direct input (number input, color picker button, text area, font dropdown)
...
- **Deselecting a font is only allowed if the font is unused** — i.e., no Text layer has `font_family` set to `{ type: "value", value: fontName }` referencing that font.
- **If a font is in use** and the user attempts to deselect it, open a modal that:
  ...
  3. On confirmation: replaces all occurrences of that font in `template.layers` with the chosen replacement, then removes the font from `fontPool`.
...
Print is **not available until all images referenced in the data have been loaded** into `state.images`. The Print menu action should be disabled (grayed out) whenever any `ImageData` column value in any data row refers to a filename not present in `state.images`.
...
|---|---|---|
| Download All | Serialize `{ template, schema, fontPool }` as JSON and `data` as CSV (two-table format), download as ZIP | |
...
| Print | Open new tab with CSS print layout of all cards | Disabled until all images loaded |
| Load Template | File picker → parse JSON → populate `template`, `schema`, `fontPool` | Rejected if `font_pool` contains any name not in `sp.svengali.Fonts` |
