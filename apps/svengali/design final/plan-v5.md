# Svengali Implementation Plan v5
...
In all cases: the **entire file is rejected** on any validation error — individual offending rows are **not** silently filtered or skipped. The error modal must identify the specific field, row, and value that caused each failure. For CSV uploads with many errors, the modal must also offer a **Download Error Report** button that produces a plain-text or CSV file listing every error (one per line, with row number, column, and description).
...
| Load Data | File picker → parse CSV → populate `data` and optionally `images` from second table | Only available after a template is loaded; rejected with error modal on any validation failure (see §16); additionally blocked if the resulting `data` would be empty while any layer field is in column mode |
