# Svengali Implementation Plan v6
...
- `featureForAddNew.md` — "Add New" pattern: behavior when a required list (font pool, images, schema columns) is empty
- `initSchemaWizardFeature.md` — Initialize Schema wizard: bulk-create schema fields and first data row from a raw template
...
> Note: Optional color fields always have the full `ColumnableField<color>` form — they are never bare `null`. A `null` inner value (`{ type: "value", value: null }`) means the color is disabled (not rendered). This ensures optional colors can be toggled to column mode like any other `ColumnableField`. Clearing a color via right-click on the color picker button sets the field to `{ type: "value", value: null }` (keeping the `ColumnableField` wrapper, setting only the inner `value` to `null`).
...
**Validation on upload**: If `fontPool` contains any font name not present in the `sp.svengali.Fonts` list, the file is deemed **invalid** and upload is rejected. All validation errors must be clearly communicated to the user (see §16 Validation Error Reporting).
  ...
  "layers": [
    {
      ...
      "background_color": { "type": "value", "value": null },
      "frame_color":      { "type": "value", "value": null }
    },
    {
      "type": "Text",
      "name": "Layer 2",
...
| Property | Default |
|---|---|
| `name` | `"Layer {n}"` where `n` is the 1-based layer count at the time of creation |
| `frame_x` | `cardWidth / 4` |
...
| Image `rotation` | `0` |
| Image `background_color` | `{ type: "value", value: null }` — color disabled (not rendered) |
| Image `frame_color` | `{ type: "value", value: null }` — color disabled (not rendered) |
...
| Text `background_color` | `{ type: "value", value: null }` — color disabled (not rendered) |
| Text `frame_color` | `{ type: "value", value: null }` — color disabled (not rendered) |
...
- When `data` is non-empty but no compatible schema field of the required type exists, a *+ Add column…* affordance is shown instead of the toggle button. See `featureForAddNew.md` §4 for the full creation flow.
- When **both** `data` is empty **and** no compatible schema field of the required type exists, neither the toggle button nor the *+ Add column…* trigger is shown. Users building a template from scratch should use **Initialize Schema…** from the menu bar to bootstrap the schema and first data row simultaneously. See `initSchemaWizardFeature.md`.
- Toggle button switches between modes and updates the `ColumnableField` discriminant

**`data` field (Image layer) in value mode**: renders a dropdown listing all currently loaded images (by filename). The selected entry stores the filename as `{ type: "value", value: filename }`. If no images are loaded when an Image layer is first added, a *+ Load image…* trigger is shown instead of the dropdown; engaging it opens the load-image flow (see `featureForAddNew.md` §3). If the user exits that flow without loading any image, the newly added Image layer is **removed** (the add is cancelled).

**Toggling `data` back to value mode with no images loaded**: If the user manually toggles an existing `data` field from column mode back to value mode and `state.images` is empty, the same *+ Load image…* trigger is shown. If the user exits without loading any image, the toggle is **reverted** and the field remains in column mode.
...
**Adding a Text layer with an empty font pool**: If `fontPool` is empty when the user adds a Text layer, the `font_family` control shows a *+ Add font…* trigger. See `featureForAddNew.md` §2 for the full creation flow. The layer is not created until a font is selected; cancelling the flow cancels the layer addition.
...
3. Offers the user a choice for each unique field type involved:
   - **Replace with another column**: a dropdown of compatible schema fields (same type, excluding the one being deleted). If no compatible field exists (the dropdown would be empty), the *+ Add column…* trigger (from `featureForAddNew.md` §4) is shown in its place, allowing a new compatible schema field to be created without leaving the guard modal.
   - **Set to a fixed value**: an appropriate value input for the property type (number, color picker, font dropdown, image filename dropdown, etc.). When the required list for this input is empty, the "Add New" pattern from `featureForAddNew.md` applies — the input is replaced by the appropriate trigger (*+ Load image…*, *+ Add font…*) and the guard modal remains open throughout the creation flow. See `featureForAddNew.md` §3 for the image-specific flow.
...
- `fontPool` entries not present in `sp.svengali.Fonts`
...
- `Font`-type column values that are not present in the current `fontPool`

In all cases: the **entire file is rejected** on any validation error — individual offending rows are **not** silently filtered or skipped. The error modal must identify the specific field, row, and value that caused each failure. Both template JSON upload errors and data CSV upload errors must offer a **Download Error Report** button that produces a `.csv` file:

- **JSON error report columns**: `path`, `value`, `error` — where `path` is the JSON key path to the offending value (e.g. `template.layers[0].background_color`).
- **CSV error report columns**: `row`, `column`, `value`, `error` — where `row` is 1-based (excluding the header row).

One error per row; the file is downloadable even when only a single error is present.
...
|---|---|---|
| Initialize Schema… | Open the Initialize Schema wizard to bulk-create schema fields and a first data row from the current template layers | Only available when `state.schema.fields` is empty AND `state.data` is empty AND `state.template.layers` is non-empty. See `initSchemaWizardFeature.md` |
...
| Load Data | File picker → parse CSV → populate `data` and optionally `images` from second table | Only available when `state.template.orientation` is non-null and `state.schema.fields` is non-empty; rejected with error modal on any validation failure (see §16); additionally blocked if the resulting `data` would be empty while any layer field is in column mode |

### Load Data — Column-Mode Guard

Before committing a Load Data result, check whether `data` would be empty (zero rows) while any `ColumnableField` across any layer is in column mode. If so, **block the load entirely** and display an error modal explaining that all column-mode fields must be reverted to value mode before loading an empty data set. This check is applied after the round-trip validation described in §16 — validation failures already reject the file before this check is reached.
...
**Removing images**: There are no per-row **Del** buttons. Instead, a single **"Remove Unused"** button sits alongside the **"Load Image"** button at the top of the tab. Clicking it removes every entry from `state.images` that is not referenced anywhere in the current session — specifically, an image is considered **in use** if its filename appears in either:
1. any value-mode `Image.data` field across all template layers, **or**
2. any data-row value in an `ImageData`-type column in `state.data`.

An image that is not found in either location is removed. This ensures that value-mode layer references and data-row image references are both preserved, keeping it impossible for any resolvable reference to be broken by "Remove Unused" (see §14).
...
| `ImageData` | Dropdown listing filenames from `state.images`; shows *+ Load image…* trigger when `state.images` is empty (see `featureForAddNew.md` §3) |
| `Font` | Dropdown restricted to the current `fontPool` entries; shows *+ Add font…* trigger when `fontPool` is empty (see `featureForAddNew.md` §2) |
